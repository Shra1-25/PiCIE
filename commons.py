...
def run_mini_batch_kmeans(args, logger, dataloader, model, view):
    ...
    with torch.no_grad():
        for i_batch, (indice, image) in enumerate(dataloader):
            ...
            if view == 1:
                image = eqv_transform_if_needed(args, dataloader, indice, image.cuda(non_blocking=True) if args.device=='cuda' else image)
                feats = model(image)
            elif view == 2:
                image = image.cuda(non_blocking=True) if args.device=='cuda' else image
    ...
    centroids = torch.tensor(centroids, requires_grad=False).cuda() if args.device=='cuda' else torch.tensor(centroids, requires_grad=False)
