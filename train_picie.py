...
def train(args, logger, dataloader, model, classifier1, classifier2, criterion1, criterion2, optimizer, epoch, scheduler):
    ...
    model.train()
    if args.mse:
        criterion_mse = torch.nn.MSELoss().cuda() if args.device=='cuda' else torch.nn.MSELoss()
    ...
    for i, (indice, input1, input2, label1, label2) in enumerate(dataloader):
        ...
        label1 = label1.cuda(non_blocking=True) if args.device=='cuda' else label1 
        featmap1 = model(input1)
        
        input2 = input2.cuda(non_blocking=True) if args.device=='cuda' else input2
        label2 = label2.cuda(non_blocking=True) if args.device=='cuda' else label2
    ...
    if not args.eval_only:
        ...
        for epoch in range(args.start_epoch, args.num_epoch):
            ...
            if not args.no_balance:
                criterion1 = torch.nn.CrossEntropyLoss(weight=weight1).cuda() if args.device=='cuda' else torch.nn.CrossEntropyLoss(weight=weight1)
                criterion2 = torch.nn.CrossEntropyLoss(weight=weight2).cuda() if args.device=='cuda' else torch.nn.CrossEntropyLoss(weight=weight2)
            else:
                criterion1 = torch.nn.CrossEntropyLoss().cuda() if args.device=='cuda' else torch.nn.CrossEntropyLoss()
                criterion2 = torch.nn.CrossEntropyLoss().cuda() if args.device=='cuda' else torch.nn.CrossEntropyLoss()
